...
def emb_size(config):
    try:
        ...
        pass
    return config.get('embedding_size',config.get('hidden_size'))
...
class Adapter(transformers.PreTrainedModel):
    ...
    def __init__(self, config, classifiers=None, Z=None, labels_list=[]):
        ...
        embedding_size = emb_size(config)
        self.Z= torch.nn.Embedding(len(config.classifiers_size),embedding_size, max_norm=1.0).weight if Z==None else Z
...
class Model(transformers.PreTrainedModel):
    def __init__(self, tasks, args, warm_start=None):
        ...
        self.torch_dtype = args.get('torch_dtype',None)
        ...
        for i, task in progress(list(enumerate(tasks))):
            ...
            model = deep_copy_cache(model_type.from_pretrained)(args.model_name,
                ignore_mismatched_sizes=True, torch_dtype=self.torch_dtype, **nl)

            if task.task_type=='MultipleChoice':
                if args.get('share_mc',False):
                    key=task.task_type
                else:
                    key=f'mc{i}'
                ...
                if not labels or type(labels[0])==int:
                    key=None
                else:
                    key= tuple([normalize_label(x) for x in labels])
        ...
        embedding_size=emb_size(self.shared_encoder.config)
    ...
    def factorize(self, task_index=0, tasks=[]):
        ...
        if not hasattr(m_i,"factorized"):
            ...
            m_i=remove_cls(m_i)
            m_i.factorized=True
        self.m_i, self.adapter = m_i, adapter
        return m_i, adapter
...
class Trainer(transformers.Trainer):
    def __init__(self, model, tasks, hparams, tokenizer=None, *args, **kwargs):
        ...
        if 'max_length' in hparams:
            for t in tasks:
                t.tokenizer_kwargs = frozendict({**t.tokenizer_kwargs, 'max_length': hparams['max_length']})
        ...
        if hparams.get('remove_overflowed_examples'):
            import xpflow
            print('Removing overflowed examples')
            for split_dataset in self.processed_tasks.values():
                for split, dataset in split_dataset.items():
                    if 'train' not in split:
                        continue
                    with xpflow.NoTqdm():
                        split_dataset[split] = dataset.filter(lambda x: x['attention_mask'].sum() < hparams['max_length'])
    ...
    def save_model(self,output_dir,task_index=0,push_adapter=True,**kwargs):
        model, adapter = self.model.factorize(task_index=task_index)
        model.save_pretrained(output_dir)
        if push_adapter:
            adapter.save_pretrained(f"{output_dir}-adapter", safe_serialization=False)
    ...
    def push_to_hub(self, repo, task_index=0, push_adapter=True):
        ...
        if push_adapter:
            adapter.push_to_hub(f"{repo.replace('-nli','')}-adapters",safe_serialization=False)
